...
# 
# The `percentile` function is part of the `datascience` package of functions. Before we can call the `percentile` function, we need to import the `datascience` package. And before we can import the `datascience` package, we need to install it on our computer since it doesn't come preloaded in Anaconda Python. Fortunately, we only need to install the `datascience` package once (ever) on our computer. To do this, run the code in the following cell. You'll need an active internet connection. The command may take a minute or two to complete and it may print a fair bit of text to the screen, which you can ignore. 
...
# The 50th percentile of the incomes is important since it can be interpreted as the income of the average household in our sample. You may want to think about which is higher: the income of the average household or the average income of all households in the US. We'll come back to this question when we examine skewed distributions later in the course.
# 
...
# 
# 1. Data about the racial composition of the households in our sample.  
# 2. Similar income data from a large sample of households in the US in 2009.  
# 3. Tax rates for various income ranges in 2019.  
# 4. Education levels for the individuals living in our sample's households.    
...
# * What efforts were made to ensure that the households were reflective of American society? 
